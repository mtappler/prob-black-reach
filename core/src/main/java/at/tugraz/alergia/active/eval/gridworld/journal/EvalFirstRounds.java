 ...
 *******************************************************************************/
package at.tugraz.alergia.active.eval.gridworld.journal;
...
public class EvalFirstRounds {
	...
	public static void main(String[] args) throws Exception {
		...
		Adapter adapter = new MatrixExportAdapter("src/main/resources/gridworld/diff_walls/diff");

		String propertiesFile = "src/main/resources/gridworld/diff_walls/walls.props";

		int property = 1;
		String path = "log/eval_each_round/log_first_gridworld_d10_more_rounds";
		logFileName = path + ".log";
		List<List<Double>> allEvaluations = new ArrayList<>();
		for (long seed : seeds) {
			System.out.println("SEED: " + seed);
			ActiveTestingStrategyInference inferrer = EvalFirst.incremental(adapter, inputs, prismLocation);
			inferrer.setMaxNrRounds(150);
	...
	private static void createMeanGraph(List<List<Double>> allEvaluations) {
		...
		for (int round = 0; round < EvalFirst.ROUNDS; round++) {
