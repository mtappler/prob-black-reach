...
public class Adversary implements AdversaryI {
	...
	public Adversary(String advFileName, String concreteModelName) throws IOException {
		Pair<TransitionHeader, Map<Long, Set<Transition>>> transitionsFileContent = importer
				.readTransFile(concreteModelName + ConcreteModelImporter.TRANS_FILE_EXT);
		concMTransitions = transitionsFileContent.getRight();
		Pair<StateHeader, Map<Long, State>> stateFileContent = importer
				.readStatesFile(concreteModelName + ConcreteModelImporter.STATE_FILE_EXT);
		states = stateFileContent.getRight();
		Pair<LabelHeader, Map<Long, StateLabels>> labelFileContent = importer
				.readLabelsFile(concreteModelName + ConcreteModelImporter.LABEL_FILE_EXT);
	...
	public void executeStep(String input, String output) {
		...
		List<Long> outputLabelIds = Arrays.stream(output.split("&")).filter(prop -> !prop.trim().isEmpty())
				.map(outputProposition -> {
					Long id = labelHeader.getTextualLabelToId(outputProposition.trim());
					return id;
				}).sorted().collect(Collectors.toList());
		List<State> statesForInputOutput = transFromState.stream().filter(t -> t.getInput().equals(input))
				.map(t -> states.get(t.getTargetId()))
				.filter(s -> labels.get(s.getId()).getLabelIds().equals(outputLabelIds)).collect(Collectors.toList());
		nrSteps++;
...
//				 System.out.println(String.format("No optimal input after %d steps in state %d",nrSteps,
//						 currentState.getId()));
				// chaos = true;
				return Optional.empty();
			} else {
