...
public class Property {
	...
	private static final Pattern innerRegex = 
			Pattern.compile("!?\\(?F\\s*\\((\\s*(\"\\w+\"\\s*&\\s*)+)\\s*(\\s*steps\\s*<\\s*(\\d+))?\\s*\\)\\s*\\)?");
	...
	public String toString(){
		...
		return String.format("P%s=?[%s(F(%s%s))]", max ? "max" : "min", 
				safety ? "!" : "",
	...
	private void parseProperty(String propertyLine) {
		...
		if(!innerMatcher.matches())
			throw new IllegalArgumentException("Unknown syntax (inner): " + actualProperty + " for " + innerRegex.toString());
		
		setLabels(Arrays.stream(innerMatcher.group(1).trim().split("&"))
	...
	public<S extends Step> boolean evaluate(FiniteString<S> trace){
		...
		boolean containsLabel = 
				relevantPart.getContent().stream()
				.anyMatch(s -> propSatisfiedInStep(s.getOutputSymbol())) ||
				propSatisfiedInStep(trace.getInitialOutput());
